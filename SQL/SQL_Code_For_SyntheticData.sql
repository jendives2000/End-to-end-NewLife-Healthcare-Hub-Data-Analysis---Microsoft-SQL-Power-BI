...
-- Create a  table along with the columns names and data type
...
--Declaring variables for use in the loop
...
BEGIN
    ...
    WHILE @j <= 2000000
    BEGIN
        SET @date = DATEADD(DAY, @j, '2024-01-01');  --start date
        SET @totalAdmissions = ABS(CHECKSUM(NEWID())) % 150 + 50; --Random daily admissions between 50 and 200
        SET @readmissions = ABS(CHECKSUM(NEWID())) % @totalAdmissions / 10; --Random daily readmissions
        SET @infections = ABS(CHECKSUM(NEWID())) % 5; -- Random daily infections (0-4)
        SET @totalDeaths = ABS(CHECKSUM(NEWID())) % 3; -- Random daily deaths (0-2)
