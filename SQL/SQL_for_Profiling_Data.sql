--dropping the #1 table in case a rerun of the SQL code is needed
drop table if exists #1

--select * from ClinicalData

--getting basic metadata on the table
...
--@columnname is the variable holding column each column name iteratively in the loop
declare @columnname nvarchar(max)
declare @datatype nvarchar(max)

--@sql is the variable holding each SQL command for each columnname, iteratively in the loop
...
--SECTION FOR CALCULATING THE MEDIAN
declare @l int
